...
def _retrive(client_socket, HEADERSIZE):
    ...
    if not len(message_header):
        return False
    ret = message(has_file=int(message_header)>1, HEADER_SIZE=HEADERSIZE)
...
def retrive(client_socket, HEADERSIZE, name):
    ...
    client_socket.send(message.get_stream(True))
...
def send(client_socket, HEADERSIZE, msg):
    ...
    if resp.check_integrity(outside_key_full) and dif.second < 3:
